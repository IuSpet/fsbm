...
func getShopInfoList() (res []mapShopInfo, err error) {
	...
	sql := "select a.id as shop_id,a.name as shop_name, b.id as user_id, b.name as user_name,b.phone as user_phone,latitude,longitude " +
		"from shop_list a join user_account_info b on a.user_id = b.id " +
		"where a.status = 0"
	err = conn.Exec(sql).Find(&res).Error
	return
}
